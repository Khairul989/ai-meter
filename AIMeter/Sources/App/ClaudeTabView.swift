...
struct ClaudeTabView: View {
    ...
    @EnvironmentObject private var claudeProxyRoutingManager: ClaudeProxyRoutingManager
    let timeZone: TimeZone
    var planName: String?
    ...
    var body: some View {
        ...
        VStack(spacing: 10) {
            ...
            if codexAuthManager.accounts.isEmpty == false {
                ClaudeRoutingControlView(
                    codexAccountCount: codexAuthManager.accounts.count,
                    routingManager: claudeProxyRoutingManager
                )
            }
...
private struct ClaudeRoutingControlView: View {
    private enum RouteOption: String, CaseIterable, Identifiable {
        case claude
        case chatgpt
        ...
        var title: String {
            ...
            case .chatgpt:
                return "ChatGPT via AIMeter"
            }
        }
    }

    let codexAccountCount: Int
    @ObservedObject var routingManager: ClaudeProxyRoutingManager
    @State private var selectedRoute: RouteOption = .claude
    @State private var showingRouteGuidance = false

    var body: some View {
        ClaudeSectionCard(
            title: "Next Session Route",
            subtitle: nil,
            surfaceColor: ClaudeTelemetryTheme.panelRaised
        ) {
            VStack(alignment: .leading, spacing: 8) {
                HStack(alignment: .center, spacing: 8) {
                    ...
                    ClaudeBadge(text: proxyStatusLabel, color: proxyStatusColor)

                    if codexAccountCount > 1 {
                        ClaudeBadge(text: "Failover", color: .green)
                    }
                }
                ...
                HStack(alignment: .center, spacing: 10) {
                    ...
                    .controlSize(.small)
                    .disabled(codexAccountCount == 0 || isBusy || selectedRoute == configuredRoute)
                }

                HStack(spacing: 8) {
                    if selectedRoute != configuredRoute {
                        Text("Pending apply")
                            .font(.system(size: 10, weight: .medium))
                            .foregroundColor(ClaudeTelemetryTheme.amberSoft)
                    } else {
                        Text(configuredRouteSummary)
                            .font(.system(size: 10))
                            .foregroundColor(ClaudeTelemetryTheme.tertiaryText)
                            .lineLimit(1)
                    }
                    ...
                    if let activeAccountEmail = routingManager.activeAccountEmail, !activeAccountEmail.isEmpty {
                ...
                if let lastError = routingManager.lastError, !lastError.isEmpty {
                    Text(lastError)
                        .font(.system(size: 10))
        ...
        .onChange(of: routingManager.isEnabled) { _, _ in
            selectedRoute = configuredRoute
        }
    }

    private var isBusy: Bool {
        if case .starting = routingManager.toggleState {
            return true
        }
        return false
    }

    private var configuredRoute: RouteOption {
        routingManager.isEnabled ? .chatgpt : .claude
    }

    private var proxyStatusLabel: String {
        switch routingManager.toggleState {
        case .off:
            return "Proxy idle"
        case .starting:
            return "Applying"
    ...
    private var proxyStatusColor: Color {
        switch routingManager.toggleState {
        case .off:
            return ClaudeTelemetryTheme.secondaryText
        case .starting:
            return ClaudeTelemetryTheme.amber
        case .on:
            return .green
        case .failed:
            return .orange
    ...
    private func applySelectedRoute() {
        let shouldEnable = selectedRoute == .chatgpt
        showingRouteGuidance = true
        Task { await routingManager.setEnabled(shouldEnable) }
    }

    private func routeChip(_ option: RouteOption) -> some View {
        let isSelected = selectedRoute == option
        let isConfigured = configuredRoute == option

        return Button {
            selectedRoute = option
        } label: {
            HStack(spacing: 6) {
                Text(option == .claude ? "Claude" : "ChatGPT")
                    .font(.system(size: 11, weight: .semibold))
                if isConfigured {
                    Circle()
                        .fill(isSelected ? Color.white.opacity(0.9) : routeTint(for: option))
                        .frame(width: 5, height: 5)
                }
            }
            .foregroundColor(isSelected ? Color.white : ClaudeTelemetryTheme.secondaryText)
            .padding(.horizontal, 10)
            .padding(.vertical, 6)
            .background(
                RoundedRectangle(cornerRadius: 9, style: .continuous)
                    .fill(isSelected ? routeTint(for: option) : Color.black.opacity(0.12))
            )
            .overlay(
                RoundedRectangle(cornerRadius: 9, style: .continuous)
                    .stroke(isSelected ? routeTint(for: option).opacity(0.85) : ClaudeTelemetryTheme.edge, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
        .disabled(codexAccountCount == 0 || isBusy)
    }

    private func routeTint(for option: RouteOption) -> Color {
        switch option {
        case .claude:
            return ClaudeTelemetryTheme.metal
        case .chatgpt:
    ...
    private var guidanceButton: some View {
        ...
        .buttonStyle(.plain)
        .help(guidanceTooltipText)
        .popover(isPresented: $showingRouteGuidance, arrowEdge: .top) {
            routingGuidancePopover
        }
    }

    private var routingGuidancePopover: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if let note = routingManager.switcherStatus.note {
                Text(note)
                    .font(.system(size: 10))
                    .foregroundColor(ClaudeTelemetryTheme.tertiaryText)
                    .fixedSize(horizontal: false, vertical: true)
            ...
            if shouldShowReloadCommand {
                HStack(spacing: 8) {
                    ...
                    Button("Copy") {
                        ...
                        NSPasteboard.general.setString("source ~/.zshrc", forType: .string)
                    }
    ...
    private var nextStepText: String {
        if !routingManager.switcherStatus.aiMeterBridgeInstalled {
            return "After your first apply, open a new terminal to pick up AIMeter's Claude launcher bridge. Existing terminals can also run the reload command once."
        }

        switch configuredRoute {
        case .claude:
            return "New terminals will launch Claude with your normal Claude subscription. Existing routed terminals keep their current session until you start a fresh Claude launch."
        case .chatgpt:
            return "Open a new terminal, then launch `claude` to use your ChatGPT subscription through AIMeter. Existing Claude sessions keep their current route until restarted."
        }
    }

    private var shouldShowReloadCommand: Bool {
        !routingManager.switcherStatus.aiMeterBridgeInstalled || configuredRoute == .chatgpt
    }

    private var guidanceTooltipText: String {
        "Applies only to future `claude` launches. Click for reload and routing guidance."
    }

    private var configuredRouteSummary: String {
        switch configuredRoute {
        case .claude:
            return "New `claude` launches use Claude."
        case .chatgpt:
            return "New `claude` launches route via AIMeter."
        }
    }
}
